--TEST--
/home/weyrick/pcc/tests/class-parent.php (converted from Roadsend suite)
--FILE--
<?php
...
?>
--EXPECTF--
This is foo in foo.  Static method invocations can't count on properties, so no zot.
This is foo in foo.  Static method invocations can't count on properties, so no zot.
This is foo in foo.  Static method invocations can't count on properties, so no zot.
foo->anothermethod: 1 and 2
bar->onemethod: 1 and 32
foo->anothermethod: 2 and 32
bar->anothermethod: 1 and 32
bar
foo
foo
