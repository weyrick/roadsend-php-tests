...
<?php
	...
	$des=new CDESCrypt();
	$test="CLEARTEXT message TO be ENCRYPTED 3-way WITH des";
	echo "Cleartext:$test\n";
	$key='';
//	srand(1);
	//for($i=0;$i<24;$i++)
//		$key.=chr(rand(0,127));
	...
	$enc=$des->des($key,$test,1,1,'potatochip',3);
	echo "Cypher:".base64_encode($enc)."\n";
	$dec=$des->des($key,$enc,0,1,'potatochip',3);
	echo "Decoded:$dec\n"
?>
--EXPECT--
Cleartext:CLEARTEXT message TO be ENCRYPTED 3-way WITH des
Key: MTIzNDU2Nzg5MDEyMzQ1Njc4OTAxMjM0
Cypher:J/JM+/pUisxLEXPsk6cooji4p94FR8KoxNiviP3cjsczLZKqK/d2Zdh1BP6Ymbnb
Decoded:CLEARTEXT message TO be ENCRYPTED 3-way WITH des
